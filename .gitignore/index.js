...
//UBIQ
    ...
    if (message.content === prefix + "pUBIQ") {
        coinmarketcap.multi(coins => {
            var maintenant=new Date();
            var jour=maintenant.getDate();
            var mois=maintenant.getMonth()+1;
            ...
            if (coins.get("UBIQ").percent_change_1h < 0) {
                pc1 = ":chart_with_downwards_trend:";
            }else {pc1 = pc1};
            
            if (coins.get("UBIQ").percent_change_24h < 0) {
                pc2 = ":chart_with_downwards_trend:";
            }else {pc2 = pc2};

            if (coins.get("UBIQ").percent_change_7d < 0) {
                pc3 = ":chart_with_downwards_trend:";
            }else {pc3 = pc3};

            message.channel.sendMessage("*" + jour + "-" + mois + "-" + an + " (" + heure + ":" + minute + ":" + seconde +")*\n" + "__**" + coins.get("UBIQ").name + "**__" + "\nRank: " + coins.get("UBIQ").rank + "\nPrice : " + coins.get("UBIQ").price_usd + " :dollar: (USD)\n" + "Percent Change 1h : " + coins.get("UBIQ").percent_change_1h + " %" + pc1 + "\nPercent Change 24h : " + coins.get("UBIQ").percent_change_24h + " %" + pc2 + "\nPercent Change 7d : " + coins.get("UBIQ").percent_change_7d + " %" + pc3 );// Prints price of UBIQ in USD
            console.log("Commande $pUBIQ effectuée");
